...
public abstract class AbstractPlayerInteraction {
    ...
    public final void warp(final int map, final int portal) {
        ...
        if (portal != 0 && map == c.getPlayer().getMapId()) { //test
            ...
            if (portalPos.distanceSq(getPlayer().getPosition()) < 90000.0) { //estimation
                c.sendPacket(MaplePacketCreator.instantMapWarp((byte) portal)); //until we get packet for far movement, this will do
    ...
    public final void warp(final int map, String portal) {
        ...
        if (map == c.getPlayer().getMapId()) { //test
            ...
            if (portalPos.distanceSq(getPlayer().getPosition()) < 90000.0) { //estimation
                c.getPlayer().checkFollow();
                c.sendPacket(MaplePacketCreator.instantMapWarp((byte) c.getPlayer().getMap().getPortal(portal).getId()));
    ...
    public final void playPortalSE() {
        c.sendPacket(MaplePacketCreator.showOwnBuffEffect(0, 7));
    }
    ...
    public void gainPet(int id, String name, int level, int closeness, int fullness, long period, short flag) {
        ...
        try {
            MapleInventoryManipulator.addById(c, id, (short) 1, "", MaplePet.createPet(id, name, level, closeness, fullness, MapleInventoryIdentifier.getInstance(), id == 5000054 ? (int) period : 0, flag), 45);
