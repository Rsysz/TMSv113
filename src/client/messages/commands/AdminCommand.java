...
public class AdminCommand {
    ...
    public static class GiveMeso extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
		    ...
		    if (Integer.MAX_VALUE - chrs.getMeso() < Integer.parseInt(splitted[2])) {    
                	c.getPlayer().dropMessage(6, "超過最大楓幣上限");
		    } else {
            		chrs.gainMeso(Integer.parseInt(splitted[2]), true);
		    }
            }
            return 1;
        }
    ...
    public static class GiveCash extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length < 3) {
                c.getPlayer().dropMessage(6, "!givecash <player> <amount>.");
                return 0;
            }
            MapleCharacter chrs = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
            if (chrs == null) {
            	...
            	chrs.modifyCSPoints(1, Integer.parseInt(splitted[2]), true);
            }
            return 1;
        }
    }
